...
public class CallActivity extends LinphoneGenericActivity implements OnClickListener, ActivityCompat.OnRequestPermissionsResultCallback, TransactionBuilder {
    ...
    private ImageView /*pause,*/ hangUp, dialer, video, /*micro,*/ /*speaker,*/
            options, /*addCall,*/
            transfer, /*conference,*/
            conferenceStatus;
    private CircularImageView contactPicture;
    private RelativeLayout speaker, pause, micro, add_call_rl, merge_call_rl;
    private BitVaultFont speaker1, pause1, micro1, addCall, conference;
    ...
    private String mTransactionSuccess = "";
    private boolean enable_pause = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        android.util.Log.d("CallActivity-", " onCreate ");
        ...
        SDKUtils.showLog(CallActivity.class.getSimpleName(), "++isCallIncoming++" + Preferences.getInstance(this).isCallIncoming());
        if (!Preferences.getInstance(this).isCallIncoming()) {
            Preferences.getInstance(this).setCallOutgoingCall(true);
        }
        timerForBitCoinDeduction();
...
//        			}
    ...
    private void timerForBitCoinDeduction() {
        Timer mBitCoinTimer = new Timer();

        mBitCoinTimer.scheduleAtFixedRate(new TimerTask() {
            @Override
            public void run() {
//                if (bitcointime==counter){
//
//                }
            }
        }, 1000, 1000);
    ...
    private void initUI() {
        ...
        pause1 = (BitVaultFont) findViewById(R.id.pause1);
        ...
        //Options
        addCall = (BitVaultFont) findViewById(R.id.add_call);
        add_call_rl = (RelativeLayout) findViewById(R.id.add_call_rl);
//        addCall.setOnClickListener(new OnClickListener() {
//            @Override
//            public void onClick(View v) {
//                AppConstants.mConferenceManagerListener.addConference();
...
//        });
        ...
        add_call_rl.setOnClickListener(this);
        ...
        conference = (BitVaultFont) findViewById(R.id.conference);
        merge_call_rl = (RelativeLayout) findViewById(R.id.merge_call_rl);
...
//		switchCamera.setOnClickListener(this);
    ...
    private void refreshIncallUi() {
        SDKUtils.showLog("DEBUG PAUSE","refreshIncallUi called");
        refreshInCallActions();
...
//            micro.setImageResource(R.drawable.micro_default);
    ...
    private void enableAndRefreshInCallActions() {
        SDKUtils.showLog("DEBUG PAUSE","enableAndRefreshInCallActions called");
        int confsize = 0;

        if (LinphoneManager.getLc().isInConference()) {
            confsize = LinphoneManager.getLc().getConferenceSize() - (LinphoneManager.getLc().isInConference() ? 1 : 0);
            SDKUtils.showLog("DEBUG PAUSE","enableAndRefreshInCallActions called: "+confsize);
        }
        ...
        if (LinphoneManager.getLc().getCallsNb() > 1 && LinphoneManager.getLc().getCallsNb() > confsize && !LinphoneManager.getLc().soundResourcesLocked()) {
            SDKUtils.showErrorLog("CallActivity", "--------Enable conference button--------");
            enabledConferenceButton(true);
        } else {
            SDKUtils.showErrorLog("CallActivity", "--------Disble conference button--------");
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.video) {
            ...
            /*if (!Preferences.getInstance(mActivity).isCallIncoming()) {*/
                SDKUtils.showErrorLog("-----DEBUG CALL-----", "++ pause call button ++");
                Intent intent = new Intent(CallActivity.this, LinphoneActivity.class);
                intent.putExtra("Transfer", false);
                startActivity(intent);
        /*  }*/
        } /*else if (id == R.id.add_call_rl) {
            SDKUtils.showErrorLog("CallActivity", "---------log1 tap on add_call button---------");
            goBackToDialer();
        } */ else if (id == R.id.pause) {
            SDKUtils.showErrorLog("DEBUG PAUSE", "++ pause call button ++");
            if (enable_pause) {
                SDKUtils.showErrorLog("DEBUG PAUSE1", "++ pause enalbe ++");
                enable_pause = false;
                LinphoneCall call = (LinphoneCall) v.getTag();
                pauseOrResumeCall(call);
            } else {
                SDKUtils.showErrorLog("DEBUG PAUSE1", "++ pause disable ++");
                enable_pause = true;
                pauseOrResumeCall(LinphoneManager.getLc().getCurrentCall());
            }
        } else if (id == R.id.hang_up) {
//            Preferences.getInstance(CallActivity.this).setCallTransactionStatus("false");
            hangUp();
            // createBitCoinTransaction(v);
        } else if (id == R.id.dialer) {
            hideOrDisplayNumpad();
        } else if (id == R.id.chat) {
            goToChatList();
        } else if (id == R.id.conference) {
            SDKUtils.showErrorLog(CallActivity.class.getSimpleName(), "---enter in conference-----");
            enterConference();
        ...
        /*else if (id == R.id.switchCamera) {
            ...
            SDKUtils.showErrorLog(TAG, "++in call pause ++");
            LinphoneCall call = (LinphoneCall) v.getTag();
            pauseOrResumeCall(call);
    ...
    private void enabledPauseButton(boolean enabled) {
        if (enabled) {
            pause.setEnabled(true);
            pause1.setBackground(null);
            pause1.setTextColor(getColor(R.color.colorH));
//            pause.setImageResource(R.drawable.pause_big_default);
        } else {
            pause.setEnabled(false);
            pause1.setBackground(getDrawable(R.drawable.icon_background));
            pause1.setTextColor(getColor(R.color.headerColor));
//            pause.setImageResource(R.drawable.pause_big_disabled);
    ...
    private void enabledConferenceButton(boolean enabled) {
        if (enabled) {
            SDKUtils.showErrorLog("CallActivity", "--------enabledConferenceButton if-------");
            conference.setVisibility(View.VISIBLE);
            merge_call_rl.setVisibility(View.VISIBLE);
            addCall.setVisibility(View.GONE);
            conference.setEnabled(true);
            add_call_rl.setVisibility(View.GONE);
        } else {
            SDKUtils.showErrorLog("CallActivity", "--------enabledConferenceButton else-------");
            conference.setVisibility(View.GONE);
            merge_call_rl.setVisibility(View.GONE);
            add_call_rl.setVisibility(View.VISIBLE);
...
//            speaker.setImageResource(R.drawable.speaker_default);
    ...
    public void pauseOrResumeCall(LinphoneCall call) {
        SDKUtils.showErrorLog("DEBUG PAUSE", " pauseOrResumeCall called");
        LinphoneCore lc = LinphoneManager.getLc();
        if (call != null && LinphoneManager.getLc().getCurrentCall() == call) {
            SDKUtils.showErrorLog("DEBUG PAUSE", " pauseOrResumeCall : "+ LinphoneManager.getLc().getCurrentCall());
...
//            pause.setImageResource(R.drawable.pause_big_over_selected);
            ...
            if (call.getState() == State.Paused) {
                SDKUtils.showLog("DEBUG PAUSE", " paused state "+State.Paused);
              //  SDKUtils.showLog(CallActivity.class.getSimpleName(), "++ resumeCall ++");
...
//            1.94191064
            ...
            BitVaultWalletManager.getWalletInstance().sendBitCoins(Preferences.getInstance(CallActivity.this).getWalletPosition(), "msm2oDJp9fCezqg2xw3qTEFKaKZP3wszd6", BTCUtils.parseValue("0.0001"),
                    BTCUtils.parseValue("0.001"), this);
        }
    }
...
//        SDKUtils.showLog(CallActivity.class.getSimpleName(), "++ isCallIncoming ++" + Preferences.getInstance(this).isCallIncoming());
//        if (!Preferences.getInstance(this).isCallIncoming()) {
//            SDKUtils.showLog(CallActivity.class.getSimpleName(), "++ Not A incoming call ++");
//            Preferences.getInstance(this).setCallIncomingCall(false);
//            Preferences.getInstance(this).setCallOutgoingCall(false);
//            Intent intent = new Intent(CallActivity.this, CallingStatus.class);
//            startActivity(intent);
//        } else finish();
    }
...
//			switchCamera.setVisibility(View.GONE);
    ...
    public void goBackToDialer() {
        SDKUtils.showErrorLog("CallActivity", "---Log2--enter in goBackToDialer----");
    ...
    protected void onResume() {
        SDKUtils.showErrorLog(TAG, "------CallActivity onResume------");
        super.onResume();
    ...
    protected void onDestroy() {
        ...
        instance = null;
        SDKUtils.showErrorLog(TAG, "----onDestroy----");
        super.onDestroy();
        System.gc();
    }
    ...
    private void displayCurrentCall(LinphoneCall call) {
        ...
        SDKUtils.showLog("----------DEBUG PAUSE------lAddress--","displayCurrentCall called" +lAddress);
...
//            displayCallStatusIconAndReturnCallPaused(null, call);
//            registerCallDurationTimer(null, call);
    ...
    private void setContactInformation(TextView contactName, ImageView contactPicture, LinphoneAddress lAddress) {
        ...
        SDKUtils.showLog("----------DEBUG PAUSE------lContact--","setContactInformation called" +lContact);
        if (lContact == null) {
            ...
            SDKUtils.showLog("----------DEBUG PAUSE------lContact.getFullName()--","setContactInformation called" +lContact.getFullName());
    ...
    private boolean displayCallStatusIconAndReturnCallPaused(LinearLayout callView, LinphoneCall call) {
        ...
        int indexId=callView.getId();
        ImageView callState = (ImageView) callView.findViewById(R.id.call_pause);
        if (callState != null) {
            SDKUtils.showErrorLog("DEBUG PAUSE", "++not null++");
        }
      //  SDKUtils.showLog("DEBUG PAUSE", " ------------Paused function called -----------"+callView.getTag().toString());
        if (LinphoneManager.getLc().getCallsNb() == 1 && indexId == 1)
        {
            LinphoneCore lc = LinphoneManager.getLc();
            lc.resumeCall(call);
            SDKUtils.showLog("DEBUG PAUSE-------", " ------------Paused function called -----------");
            // pauseOrResumeCall(call);
        ...
        if (call.getState() == State.Paused || call.getState() == State.PausedByRemote || call.getState() == State.Pausing) {
            ...
            SDKUtils.showLog("DEBUG PAUSE", "++ pauseOrResumeCall in   function++"+State.Paused);
           // pauseOrResumeCall(call);
            isCallPaused = true;
    ...
    private void registerCallDurationTimer(View v, LinphoneCall call) {
        ...
        SDKUtils.showLog(CallActivity.class.getSimpleName(), "++ callDuration ++" + callDuration);
    ...
    public void refreshCallList(Resources resources) {
        SDKUtils.showLog("DEBUG PAUSE","refreshCallList called");
        ...
        if (merge_call_rl.getVisibility() == View.VISIBLE) {
            merge_call_rl.setVisibility(View.GONE);
            add_call_rl.setVisibility(View.VISIBLE);
        }
...
//            displayCallPaused(false);
    ...
    private void displayConferenceParticipant(int index, final LinphoneCall call) {
        ...
        ImageView contactPicture= (ImageView) confView.findViewById(R.id.contactPicture);
        ...
        BitVaultFont quitConference = (BitVaultFont) confView.findViewById(R.id.quitConference);
        ...
        BitVaultFont mCancelConference= (BitVaultFont) confView.findViewById(R.id.mCancelConference);
        mCancelConference.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                LinphoneCore lc = LinphoneManager.getLc();
    ...
    public void TransactionId(String mTxId) {
        ...
        mTransactionSuccess = mTxId;
        hangUp();
        Intent intent = new Intent(CallActivity.this, CallingStatus.class);
        intent.putExtra("myTransactionId", mTransactionSuccess);
        startActivity(intent);
    }
