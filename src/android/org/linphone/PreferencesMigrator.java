...
public class PreferencesMigrator {
	...
	public void migrateRemoteProvisioningUriIfNeeded() {
		String oldUri = mNewPrefs.getConfig().getString(AppKeyHelper.KEY_APP, "remote_provisioning", null);
		...
		if (oldUri != null && oldUri.length() > 0 && currentUri == null) {
			...
			mNewPrefs.getConfig().setString(AppKeyHelper.KEY_APP, "remote_provisioning", null);
			mNewPrefs.getConfig().sync();
		}
	}

	public void migrateSharingServerUrlIfNeeded() {
		String currentUrl = mNewPrefs.getConfig().getString(AppKeyHelper.KEY_APP, "sharing_server", null);
		if (currentUrl == null || currentUrl.equals(AppConstants.NEW_USER_URL)) {
			mNewPrefs.setSharingPictureServerUrl(AppConstants.SHARING_PICTURE_SERVER);
			mNewPrefs.getConfig().sync();
		}
	}
	...
	public void doPresenceMigrationIfNeeded() {
		...
		if (cfg.getString(AppKeyHelper.KEY_APP, "friendlist_subscription_enabled", null) == null){
			...
			if (proxy != null) {
				...
				if (domain!=null && domain.equals(getString(R.string.default_domain))) {
					cfg.setBool(AppKeyHelper.KEY_APP, "friendlist_subscription_enabled", true);
				}
			}
		}
	}
